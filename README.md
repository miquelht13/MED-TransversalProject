...
As a first idea, we wanted the object detection process to be carried out on the Raspberry Pi. Due to the poor performance of it when performing the Yolo processes, we had to migrate that process to the main computer, running it in conjunction with the web app, as you will see in the last activity of this Phase.
